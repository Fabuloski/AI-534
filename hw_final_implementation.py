...
def preprocess_data(data, normalize=False, drop_sqft_living15=False, test=False):
    ...
    if normalize:
        ...
        for col in preprocessed_data.columns:
            if col != "price" and col!= "waterfront" and col != "bias":
                mu = np.mean(preprocessed_data[col])
                sigma = np.std(preprocessed_data[col])
                preprocessed_data[col]   = (preprocessed_data[col] -mu)/ sigma
                params[col] = (mu, sigma)
    if test:
         for col in preprocessed_data.columns:
            if col != "price" and col != "waterfront" and col != "bias":
...
def gd_train(data, lr, e, n_iter=4000):
    ...
    for i in range(n_iter):
        ...
        if abs(diff)<=e and abs(diff) != mse[i+1]:
            weights -= lr * grad_L
            convergence[lr] = True 
            ...
            weights -= lr * grad_L
            convergence[lr] = "Will Converge"
            continue
        
        elif diff>0 and abs(diff) != mse[i+1]:
            convergence[lr] = False
            return weights, mse[1: i+2], convergence
...
def plot_losses(losses, convergence, normalized=True):
    ...
    plt.title("Convergence of Learning steps, " + title_string, fontweight="bold")

    for (lr, loss) in losses.items():
        if convergence[lr][lr]==False:
            pass
        else:
            iterations = [j for j in range(1, len(loss)+1)]
            plt.plot(iterations, loss, label="lr=" + lr)
...
def compare_rate(data, lrs, e):
    ...
    for lr in lrs:
        (weight, mse, convergence) = gd_train(data, lr, e)
        mses[lr]              = mse
        convergences[lr]      = convergence
        weights[lr]           = weight
...
def validate(val_data, weights, convergences):
    ...
    if type(weights) is dict:
        for (lr, weight) in weights.items():
            loss = 0
            if convergences[lr][lr]==False:
                pass
            else:
                for j in range(n_data):
                    y_val = val_labels[j]
                    x_val = x_val[:,j]
                    y_pred_val = np.dot(weight, x_val)         
                    loss += (y_pred_val - y_val) ** 2
                mse[lr] = round((1/n_data) * loss, 3) 
    else:
        loss=0
        for j in range(n_data):
                y_val = val_labels[j]
                x_val = x_val[:,j]
                y_pred_val = np.dot(weights, x_val)         
                loss  += (y_pred_val - y_val) ** 2
...
weights_norm, mses_norm, convergences_norm = compare_rate(data["train_norm"], learning_rates_norm, e)

# print(losses normalized)
plot_losses(mses_norm, convergences_norm)
print("Based on the normalized data plot, convergence occurs in all the plots except lr=1")
...
best_weight = weights_norm["0.1"][1:]
print("The weights for best learning rate is" + best_weight)
print("validation MSE from the learned weights for normalized data are: " + validation_norm)
...
weights, mses, convergences = compare_rate(data["train"], learning_rates_not_normalized, e)
...
##Honing on the highest converging learning rate
plain_result = gd_train(data["valid"], lr = 9.84*(10**-11), e=0.01, n_iter = 4000)
plain_weight = plain_result[0]
plain_convergence = plain_result[2]

# validate not normalized
validation = validate(data["valid"], plain_weight, plain_convergence)
print("validation MSE from the learned weights for NON-normalized data using lr = 9.84*(10**-11) is: " + validation)
...
print("The weights for learning rate 0.1 with the redundant feature are: " + data_sqft_living_dropped_weights[1:])
...
print("validation MSE for the normalized data having dropped the redundance at learning rate 0.1 is: " + data_sqft_living_dropped_val)
